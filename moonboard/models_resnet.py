...
class VAE(nn.Module):
    ...
    def forward(self, x, c=None, testing=False):
        ...
        if self.variational or self.conditional:
            ...
            if self.conditional:
                means, var = self.encoder(x, c)
            else:
                means, var = self.encoder(x)
            ...
            if testing:
                ...
                z = means + eps.to("cuda") * std.to("cuda")
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer_sizes, latent_size, conditional, variational, num_labels):
        ...
        if self.variational or self.conditional:
            self.linear_means = nn.Linear(128*5*3, latent_size)
            self.linear_log_var = nn.Linear(128*5*3, latent_size)
        else:
            self.linear_z = nn.Linear(1920, latent_size)
...
class Decoder(nn.Module):
    ...
    def __init__(self, layer_sizes, latent_size, conditional, variational, num_labels):
        ...
        if self.conditional:
            input_size = latent_size + 3*18
        else:
            input_size = latent_size

        self.model = ResNet_inv(1, (128,5,3), [1, 1, 1, 1], latent_size=input_size)
...
class ResNet_inv(nn.Module):
    def __init__(self,
        ...
        self.fc = nn.Linear(latent_size, 1920)

        self.c1 = nn.ConvTranspose2d(32, 3, 1, stride = 1)  # b, 16, 5, 5
        self.c2 = nn.Conv2d(3, 3, (3,2), stride = 1)  # b, 16, 5, 5
